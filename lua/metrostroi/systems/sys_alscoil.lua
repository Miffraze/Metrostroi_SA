            ...
            if autostop then
                if IsValid(arsback) then
                    if arsback == self.AutostopSignal then
                        ...
                        if not nomsg then
                            RunConsoleCommand("say",Format("%s %s %s с запрещающим показанием%s",Train:GetDriverName(),arsback.InvationSignal and "проследовал светофор" or "допустил проезд светофора",arsback.Name,arsback.InvationSignal and "по пригласительному сигналу." or "."))
                        end
